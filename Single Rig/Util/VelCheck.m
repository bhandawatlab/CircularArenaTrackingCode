...
for j = 1:length(critPoints)-1
    % true when the thrust is more positive than negative
    case1 = length(find(thrustV(critPoints(j)+1:critPoints(j+1))>3))>...
        length(find(thrustV(critPoints(j)+1:critPoints(j+1))<-3));
    % true when the speed is more positive than negative
    case2 = length(find(speed(critPoints(j)+1:critPoints(j+1))>3))>...
        length(find(speed(critPoints(j)+1:critPoints(j+1))<-3));

    if ~case1 %(case1 && ~case2 || ~case1 && case2)
        if all == 1
