function [fly] = Tracking2(Inside,vertices,readframe,bgvid,bgvid2,frames,fileNum)
regions = 4;
...
for i = 1:frames %loop from start-->endframe
    ...
    for k = 1:regions
        ...
        if ~isempty(idx)
            ...
            xMajor1 = centroid(1)+minVals{k}(1) + mjrAL * cosd(angVec);
            yMajor1 = centroid(2)+minVals{k}(2) + mjrAL * sind(angVec);
