function [fly2] = PostHocAnalysis(fly,sArena,vertices,badFlies)
% addpath('C:\Nicholas Data\New Code (4-rig)\Image Processing New Rig\Util')
f = 30;
cF = sArena.cF;
...
for k = 1:4
    if any(k == badFlies) || ~isfield(fly{k},'Center')
        ...
        [startNdx,endNdx,type] = startEndSeq(isnan(x));
        if length(startNdx)>1
            startNdx =startNdx(type==1);
            endNdx =endNdx(type==1);
            for i = 1:length(startNdx)
                if endNdx(i) == length(x)
                    setNdx = startNdx(i)-1;
                else
                    setNdx = endNdx(i)+1;
                end
                x(startNdx(i):endNdx(i)) = x(setNdx);
                y(startNdx(i):endNdx(i)) = y(setNdx);
                fly{k}.ang(startNdx(i):endNdx(i)) = fly{k}.ang(setNdx);
                fly{k}.MjrAxs(startNdx(i):endNdx(i)) = fly{k}.MjrAxs(setNdx);
                fly{k}.MinAxs(startNdx(i):endNdx(i)) = fly{k}.MinAxs(setNdx);
                fly{k}.Head.x(startNdx(i):endNdx(i)) = fly{k}.Head.x(setNdx);
                fly{k}.Head.y(startNdx(i):endNdx(i)) = fly{k}.Head.y(setNdx);
            end
        end
