function [Inside,vertices] = getArenaBounds(Inside,vertices,filename)
...
if isempty(Inside)
    [Inside,vertices] = collectArenaBounds(img,headerinfo);
    ...
    if check == true
        ...
        x = input('Accept these arena bounds? [Y/N]','s');
        if strcmpi(x,'Y')
            save([filename(1:end-5) '_Inside.mat'],'Inside','vertices')
        elseif strcmpi(x,'N')
            [Inside,vertices] = collectArenaBounds(img,headerinfo);
            ...
            disp('Not a valid option, assume N')
            [Inside,vertices] = collectArenaBounds(img,headerinfo);
            save([filename(1:end-5) '_Inside.mat'],'Inside','vertices')
        end
    end
...
function [Inside,vertices] = collectArenaBounds(img,v)
